...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        # Main Inputs expander - contains all parameter groups
        with st.expander("📥 Inputs", expanded=st.session_state.get('exp_inputs', True)):

            # Project section
            with st.expander("📐 Project", expanded=st.session_state.get('exp_project', False)):
        ...
        with col1:
            if st.button("− Collapse All", use_container_width=True, help="Zatvori sve sekcije"):
                st.session_state.exp_inputs = False
        ...
        with col2:
            if st.button("+ Expand All", use_container_width=True, help="Otvori sve sekcije"):
                st.session_state.exp_inputs = True
                for k in ['exp_project', 'exp_revenue', 'exp_yield', 'exp_financing',
                          'exp_tax', 'exp_construction', 'exp_reserves', 'exp_horizon']:
                    st.session_state[k] = False
                st.rerun()
